...
export const getAllScopes = async (
  ...
  try {
    ...
    const where: any = {};
    if (search) where.scope = { [Op.like]: `%${search}%` };
    const order: any[] = [];
    ...
    return successResponse(res, "Success get all global role", scopes, {
      limit,
      offset,
      total: count,
...
export const getScopeById = async (
  ...
  try {
    ...
    if (!id) {
      return errorResponse(res, "ID tidak ditemukan", null, 404);
    }

    const scope = await Scope.findByPk(id);

    if (!scope) {
      return errorResponse(res, "Scope tidak ditemukan", null, 404);
    }

    return successResponse(res, "Success get scope by id", scope);
  } catch (error: unknown) {
    if (
      error instanceof ValidationError ||
      error instanceof UniqueConstraintError
...
export const createScope = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    return successResponse(res, "Success create scope", data);
  } catch (error: unknown) {
    if (
      error instanceof ValidationError ||
      error instanceof UniqueConstraintError
...
export const updateScope = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    await data.save();
    return successResponse(res, "Success update scope", data);
  } catch (error: unknown) {
    if (
      error instanceof ValidationError ||
      error instanceof UniqueConstraintError
...
export const deleteScope = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    await data.destroy();
    return successResponse(res, "Success delete scope", { id });
  } catch (error: unknown) {
    if (
      error instanceof ValidationError ||
      error instanceof UniqueConstraintError
