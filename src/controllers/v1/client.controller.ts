...
import { Client, ClientScope, ClientGrant, RedirectUri } from "@/repositories";
...
export const ClientControllerV1 = {
  ...
  addScope: asyncHandler(
    async (req: Request, res: Response) => {
      ...
      successResponse(res, "Success create scope", {
        id: data.id,
        service: data.Scope.Service.name,
  ...
  updateScope: asyncHandler(
    async (req: Request, res: Response) => {
      ...
      const { ClientId, ScopeId } = req.params;
      const { scope_id, action_kode } = req.body;
      const data = ClientScope.updateOne(
        {
          where: {
            id: ScopeId,
            client_id: ClientId,
  ...
  updateRedirect: asyncHandler(async (req: Request, res: Response) => {
    const t = req.transaction;
    if (!t) {
      throw new InvalidRequestError("Transaksi tidak ditemukan");
    }
